...
getEventData = async (id) => {
  let historyData = []
  await firestore().collection('booksBooking').get().then((queryShot) => {
    if (queryShot.empty) {
      ...
      <ScrollView>
        {
          this.state.historyData.map((item) => {
            ...
            >
              ...
              <Text style={style.lblDate}>{"Person Name: " + item?.name}</Text>
              <Text style={style.lblDate}>{"From Date: " + item?.fromDate}</Text>
              <Text style={style.lblDate}>{"To Date: " + item?.toDate}</Text>
            </TouchableOpacity>
            )
          })
        }
