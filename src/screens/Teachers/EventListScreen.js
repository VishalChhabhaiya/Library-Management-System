...
getEventData = async () => {
  let eventData = []
  await firestore().collection('event').get().then((queryShot) => {
      ...
      <ScrollView>
        {
          this.state.eventData.map((item) => {
            ...
            <TouchableOpacity 
                ...
                activeOpacity={1.0}
            >
              <Text style={style.lblTitle}>Event Name: {item?.name}</Text>
              <Text style={style.lblDate}>{"Date: " + item?.date}</Text>
